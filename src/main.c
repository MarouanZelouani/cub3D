...
/*   Updated: 2025/03/26 10:25:47 by mzelouan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	start_game(t_parsed_data *data)
{
	...
	p_args.img.addr = mlx_get_data_addr(p_args.img.img,
			&p_args.img.bits_per_pixel, &p_args.img.line_length,
			&p_args.img.endian);
...
int main (int ac, char **av)
{
  ...
  if (validation(&data))
    return (free_data(&data), ft_putstr_fd("naww", 2),1);
	start_game(&data);
  return 0;
}
