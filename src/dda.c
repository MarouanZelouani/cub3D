...
/*   Updated: 2025/03/23 00:50:22 by mzelouan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void get_ray_lenght(t_args *args, t_ray *ray, int ray_number)
{
    ...
    {
        ray->step.x = 1;
        ray->side_dist.x = (ray->map_pos_x + 1.0 - args->player.cords.x / TILE_HSIZE)\
         * ray->delta_dist.x;
    }  
    if (ray->dir.y < 0)
    {
    ...
    {
        ray->step.y = 1;
        ray->side_dist.y = (ray->map_pos_y + 1.0 - args->player.cords.y / TILE_HSIZE)\
         * ray->delta_dist.y;
    }
...
{
    if (ray->side == 0)
        ray->lenght = (ray->map_pos_x - (args->player.cords.x / TILE_HSIZE)\
         + (1 - ray->step.x) / 2) / ray->dir.x;
    else
        ray->lenght = (ray->map_pos_y - (args->player.cords.y / TILE_HSIZE)\
