...
/*   Updated: 2025/03/24 09:30:07 by mzelouan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/cub3D.h"

static int	check_corners(t_args *args, t_pair tile, int cord[2],
				double buffer);
		...
		double buffer)
{
	...
	if (tile.x < buffer && tile.y < buffer && map_x > 0 && map_y > 0
		...
		return (1);
	if (tile.x > 1.0 - buffer && tile.y < buffer && map_x < args->p_data->w - 1
		&& map_y > 0 && args->p_data->map[map_y - 1][map_x + 1] == '1')
		return (1);
	if (tile.x < buffer && tile.y > 1.0 - buffer && map_x > 0
		&& map_y < args->p_data->h - 1
		&& args->p_data->map[map_y + 1][map_x - 1] == '1')
		return (1);
	if (tile.x > 1.0 - buffer && tile.y > 1.0 - buffer
		&& map_x < args->p_data->w - 1
		&& map_y < args->p_data->h - 1
		&& args->p_data->map[map_y + 1][map_x + 1] == '1')
		return (1);
	return (0);
